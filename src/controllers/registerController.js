...
const { getAllRegisterModel, postRegisterModel, updateDataRegisterModel } = require('../models/registerModel')

module.exports = {
    getAllRegister: async (req, res) => {
            try {
                const result = await getAllRegisterModel()
                return response.response(res, 200, 'Success get all register data', result);
            } catch (error) {
    ...
    updateDataRegister: async (req, res) => {
        try {
            ...
            return response.response(res, 200, `Success update register data with id : ${req.params.id}`, result);
        } catch (error) {
            return response.response(res, 400, "Bad Request")
        }
    },
